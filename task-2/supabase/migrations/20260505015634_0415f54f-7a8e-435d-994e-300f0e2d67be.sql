...
RETURNS TABLE(outcome text, rsvp_id uuid, user_id uuid, display_name text, checked_in_at timestamp with time zone)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT r.* INTO _rsvp FROM public.rsvps r
  ...
  UPDATE public.rsvps AS r
   ...
   WHERE r.id = _rsvp.id
   RETURNING r.checked_in_at INTO _checked_in_at;

  RETURN QUERY SELECT 'ok'::text, _rsvp.id, _rsvp.user_id, _name, _checked_in_at;
END;
$function$;